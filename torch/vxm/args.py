...
class Args(argparse.ArgumentParser):
    ...
    def __init__(self, **overrides):
        ...
        **overrides (dict, optional): Keyword arguments used to override default argument values
        ...
        self.add_argument('--use_mask', type=bool,  default=False, help = 'use the segmentation mask to compute the dice in each epoch')
        self.add_argument('--sf', type=float, default=1, help = "Scale factor. Uses the same scale factor for down sample and upsample")
        self.add_argument('--patch_size', type=int, default=256, help = "An integer defining the size of a patch")
        self.add_argument('--use_patch', type=bool, default=False, help = "Whether to use patch or not (if True, define patch size)")



        # Setting dataset params
        self.add_argument('--train_moving_data', type=pathlib.Path, help= "Path to train  data")
        self.add_argument('--train_fixed_data', type=pathlib.Path, help="Path to train fixed data")

        self.add_argument('--val_moving_data', type=pathlib.Path, help="Path to val  data")
        self.add_argument('--val_fixed_data', type=pathlib.Path, help="Path to val fixed data")

        self.add_argument('--moved_save_path', type=pathlib.Path, help="Path to save the aligned data")
        self.add_argument('--flow_save_path', type=pathlib.Path, help="Path to save the flow (for debugging purpose) ")

        # set up path for mask
        self.add_argument('--train_moving_mask', type=pathlib.Path, help="Path to train moving mask")
        self.add_argument('--train_fixed_mask', type=pathlib.Path, help="Path to train fixed mask")

        self.add_argument('--val_moving_mask', type=pathlib.Path, help="Path to val moving mask")
        self.add_argument('--val_fixed_mask', type=pathlib.Path, help="Path to val fixed mask")
        self.add_argument('--moved_mask_save_path', type=pathlib.Path, help="Path to save the aligned data")



        # specify data format (dicom or nii)
        self.add_argument('--data_type', type=str, choices= ['dicom', 'nifti'], default="dicom", help = 'specify the type of input data. The exp_output type is infered from the input type')

        # preprocess
        self.add_argument('--norm_method', type=str, default='z-score', help = "min-max or z-score")
        self.add_argument('--save_npy_flow', type=int, default=0, help = "1 to save flow as npy and 0 to not save")
        ...
        self.add_argument('--dim', type=int, default=3, help='3 in case of 3d, 2 in case of 2d image registration')
        self.add_argument('--enc_nf', type=list, default=[32, 32, 32, 8], help='===to be inserted===')
        self.add_argument('--dec_nf', type=list, default=[32, 32, 32, 32, 16, 8], help='=== to be inserted ===')
        ...
        self.add_argument('--epochs', type=int, default=500, help='Number of training epochs')
